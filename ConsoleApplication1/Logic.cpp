...
Logic::Logic(int w, int h, int blockSize) : width(w), height(h), blockSize(20), score(0), k(1), redFruitActive(false), direction('d') {
	snake.push_back(SnakeBody(width / 2, height - 1));
...
void Logic::updateGame() {
	selfCollision();
	SnakeBody head = snake.front();
	if (redFruitActive && clk.getElapsedTime().asSeconds() > 2.5)
		redFruitActive = false;
...
void Logic::wrapSnake() {
	//SnakeBody head = snake.front();
	//if (head.x < 0) /*head.x = width - 1;*/ snake.push_front(SnakeBody(width - 1, head.y));
	//else if (head.x >= width) /*head.x = 0;*/ snake.push_front(SnakeBody(0, head.y));
	//if (head.y < 0) /*head.y = height - 1;*/ snake.push_front(SnakeBody(head.x, height - 1));
	//else if (head.y >= height) /*head.y = 0;*/ snake.push_front(SnakeBody(head.x, 0));

	//snake.front() = head;

	SnakeBody head = snake.front();
	if (head.x >= width-1 && direction == 'd') {
		snake.push_front(SnakeBody(0, head.y));
	}
	if (head.x <= 0 && direction == 'a') {
		snake.push_front(SnakeBody(width-1, head.y));
	}
	if (head.y <= 0 && direction == 'w') 
		snake.push_front(SnakeBody(head.x, height-1));
	if (head.y >= height-1 && direction == 's') 
		snake.push_front(SnakeBody(head.x, 0));
}


void Logic::selfCollision() {
	SnakeBody head = snake.front();
